...
class GraphSimulationResultTests(unittest.TestCase):
    def test_completed_result_is_successful_and_exposes_nested_samples(self):
        result = GraphSimulationResult(0, 1, .1, 2, .2, SimulationState.COMPLETED, (0, .2), {"A": {"y": (1, 2)}, "B": {}})
        self.assertEqual(result.sample_count, 2)
        self.assertTrue(result.successful)
        self.assertEqual(result.node_outputs, {"A": {"y": (1, 2)}, "B": {}})

    def test_stopped_result_is_not_successful_and_empty_node_outputs_are_valid(self):
        result = GraphSimulationResult(0, 1, .1, 0, 0, SimulationState.STOPPED, (0,), {"A": {}, "B": {}})
        self.assertEqual(result.sample_count, 1)
        self.assertFalse(result.successful)

    def test_nested_output_length_mismatch_rejects(self):
        with self.assertRaises(ValueError):
            GraphSimulationResult(0, 1, .1, 0, 0, SimulationState.STOPPED, (0,), {"A": {"y": (1, 2)}})
    def test_invalid_time_steps_and_nested_lengths_reject(self):
        cases = [
            ...
            dict(completed_steps=-1), dict(final_time=2, timestamps=(0, 2)),
        ]
        for values in cases:
