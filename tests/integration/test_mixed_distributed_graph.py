...
from farcel.contracts.worker_protocol import SetInputsRequest, WorkerMessageType
...
class _RecordingImporter:
    def __init__(self, delegate: FmpyImporter) -> None:
...
class _RecordingTransport:
    """Transparent TCP transport wrapper used only to observe real requests."""

    def __init__(self, delegate: TcpWorkerClient) -> None:
        self._delegate = delegate
        self.requests = []
    ...
    def request(self, request, *, binary_payload=None):
        ...
        return self._delegate.request(request, binary_payload=binary_payload)

    def close(self) -> None:
        self._delegate.close()


class MixedDistributedGraphIntegrationTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _mixed_result(self, graph: SimulationGraph, worker_id: str):
        ...
        with TemporaryDirectory() as temporary_directory:
            ...
            try:
                ...
                self.assertTrue(set_inputs)
                self.assertTrue(
                    any(
                        isinstance(payload, SetInputsRequest)
                        and payload.values.get(INPUT) == 2.0
                        for payload in set_inputs
                    ),
                    "A previous snapshot 的非空 routed input 必须经 TCP SET_INPUTS 发送给 B",
                )
    ...
    def _assert_mixed_parity(self, local_result, mixed_result) -> None:
        ...
        # t=0 has no coupling; each later checkpoint routes A's previous snapshot.
        for actual, expected in zip(mixed_b, (0.0, 2.0, 2.0)):
            self.assertTrue(math.isclose(float(actual), expected, abs_tol=_ABS_TOLERANCE))

    def test_mixed_local_to_worker_result_matches_local_jacobi_zoh_baseline(self) -> None:
        graph = self._graph(("A", "B"))
        local_result = self._local_result(graph)

        mixed_result = self._mixed_result(graph, "worker-mixed-forward")

        self._assert_mixed_parity(local_result, mixed_result)

    def test_mixed_result_does_not_depend_on_node_declaration_order(self) -> None:
        graph = self._graph(("B", "A"))
        local_result = self._local_result(graph)

        mixed_result = self._mixed_result(graph, "worker-mixed-reversed")
