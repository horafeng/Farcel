...
$asset = Join-Path $repositoryRoot "src\farcel\standard_library\assets\sources\sine\Sine.fmu"
$metadata = Join-Path $repositoryRoot "src\farcel\standard_library\assets\categories\sources\sine\block.json"
$stage = Join-Path ([System.IO.Path]::GetTempPath()) "farcel-standard-sine-build"
$gcc = Find-Gcc
...
& $gcc -shared -std=c11 -O2 -s -static-libgcc -o (Join-Path $stage "binaries\win64\FarcelSine.dll") (Join-Path $PSScriptRoot "source\sine.c") -lm
if ($LASTEXITCODE -ne 0) { throw "GCC 未能编译 Sine.dll" }
