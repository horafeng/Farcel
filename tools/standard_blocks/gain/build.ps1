...
$asset = Join-Path $repositoryRoot "src\farcel\standard_library\assets\math\gain\Gain.fmu"
$metadata = Join-Path $repositoryRoot "src\farcel\standard_library\assets\categories\math\gain\block.json"
$stage = Join-Path ([System.IO.Path]::GetTempPath()) "farcel-standard-gain-build"
$gcc = Find-Gcc
...
& $gcc -shared -std=c11 -O2 -s -static-libgcc -o (Join-Path $stage "binaries\win64\FarcelGain.dll") (Join-Path $PSScriptRoot "source\gain.c")
if ($LASTEXITCODE -ne 0) { throw "GCC 未能编译 Gain.dll" }
