/*
 * Farcel Standard Block Library -- Gain FMI 2.0 Co-Simulation FMU.
...
FMI2_EXPORT fmi2Component fmi2Instantiate(
    ...
    instance = (GainInstance*)calloc(1, sizeof(GainInstance));
    if (instance != NULL) {
        instance->gain = 1.0;
    }
    return instance;
...
FMI2_EXPORT fmi2Status fmi2Reset(fmi2Component component) {
    GainInstance* instance = (GainInstance*)component;
    if (instance == NULL) {
        return fmi2Error;
    }
    instance->gain = 1.0;
...
FMI2_EXPORT fmi2Status fmi2SetReal(
    ...
    for (index = 0; index < value_count; ++index) {
        if (value_references[index] == VR_GAIN) {
            instance->gain = values[index];
        } else if (value_references[index] == VR_U) {
            instance->u = values[index];
        } else {
            return fmi2Error;
        }
    }
...
FMI2_EXPORT fmi2Status fmi2GetReal(
    ...
    for (index = 0; index < value_count; ++index) {
        if (value_references[index] == VR_GAIN) {
            values[index] = instance->gain;
        } else if (value_references[index] == VR_U) {
            values[index] = instance->u;
        } else if (value_references[index] == VR_Y) {
            values[index] = instance->gain * instance->u;
        } else {
            return fmi2Error;
        }
    }
...
/* Gain exposes only scalar Real variables. The remaining FMI 2 APIs are
