...
$assetDirectory = Join-Path $repositoryRoot "src\farcel\standard_library\assets\continuous\first_order"
$asset = Join-Path $assetDirectory "FirstOrder.fmu"
$metadata = Join-Path $assetDirectory "block.json"
$stage = Join-Path ([System.IO.Path]::GetTempPath()) "farcel-standard-first-order-build"
$gcc = Find-Gcc
...
& $gcc -shared -std=c11 -O2 -s -static-libgcc -o (Join-Path $stage "binaries\win64\FarcelFirstOrder.dll") (Join-Path $PSScriptRoot "source\first_order.c") -lm
if ($LASTEXITCODE -ne 0) { throw "GCC 未能编译 FirstOrder.dll" }
