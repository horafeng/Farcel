...
$assetDirectory = Join-Path $repositoryRoot "src\farcel\standard_library\assets\continuous\integrator"
$asset = Join-Path $assetDirectory "Integrator.fmu"
$metadata = Join-Path $assetDirectory "block.json"
$stage = Join-Path ([System.IO.Path]::GetTempPath()) "farcel-standard-integrator-build"
$gcc = Find-Gcc
...
& $gcc -shared -std=c11 -O2 -s -static-libgcc -o (Join-Path $stage "binaries\win64\FarcelIntegrator.dll") (Join-Path $PSScriptRoot "source\integrator.c")
if ($LASTEXITCODE -ne 0) { throw "GCC 未能编译 Integrator.dll" }
