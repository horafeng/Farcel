/* Farcel Standard Block Library -- Integrator FMI 2.0 Co-Simulation FMU. SPDX-License-Identifier: MIT */
#include <stddef.h>
#include <stdlib.h>
...
enum { VR_INITIAL_VALUE = 0, VR_U = 1, VR_Y = 2 };

typedef struct { fmi2Real initial_value; fmi2Real u; fmi2Real y; } IntegratorInstance;
...
FMI2_EXPORT fmi2Component fmi2Instantiate(fmi2String name, fmi2Type type, fmi2String guid, fmi2String resource, const fmi2CallbackFunctions* callbacks, fmi2Boolean visible, fmi2Boolean logging) { (void)name; (void)guid; (void)resource; (void)callbacks; (void)visible; (void)logging; return type == fmi2CoSimulation ? calloc(1, sizeof(IntegratorInstance)) : NULL; }
...
FMI2_EXPORT fmi2Status fmi2ExitInitializationMode(fmi2Component c) { IntegratorInstance* x = (IntegratorInstance*)c; if (!x) return fmi2Error; x->y = x->initial_value; return fmi2OK; }
FMI2_EXPORT fmi2Status fmi2Terminate(fmi2Component c) { return c == NULL ? fmi2Error : fmi2OK; }
FMI2_EXPORT fmi2Status fmi2Reset(fmi2Component c) { IntegratorInstance* x = (IntegratorInstance*)c; if (!x) return fmi2Error; x->initial_value = 0.0; x->u = 0.0; x->y = 0.0; return fmi2OK; }
FMI2_EXPORT fmi2Status fmi2SetReal(fmi2Component c, const fmi2ValueReference refs[], size_t n, const fmi2Real values[]) { IntegratorInstance* x = (IntegratorInstance*)c; size_t i; if (!x) return fmi2Error; for (i = 0; i < n; ++i) { if (refs[i] == VR_INITIAL_VALUE) x->initial_value = values[i]; else if (refs[i] == VR_U) x->u = values[i]; else return fmi2Error; } return fmi2OK; }
FMI2_EXPORT fmi2Status fmi2GetReal(fmi2Component c, const fmi2ValueReference refs[], size_t n, fmi2Real values[]) { IntegratorInstance* x = (IntegratorInstance*)c; size_t i; if (!x) return fmi2Error; for (i = 0; i < n; ++i) { if (refs[i] == VR_INITIAL_VALUE) values[i] = x->initial_value; else if (refs[i] == VR_U) values[i] = x->u; else if (refs[i] == VR_Y) values[i] = x->y; else return fmi2Error; } return fmi2OK; }
FMI2_EXPORT fmi2Status fmi2DoStep(fmi2Component c, fmi2Real point, fmi2Real step, fmi2Boolean no_prior_state) { IntegratorInstance* x = (IntegratorInstance*)c; (void)point; (void)no_prior_state; if (!x || step < 0.0) return fmi2Error; x->y += x->u * step; return fmi2OK; }
