...
$asset = Join-Path $repositoryRoot "src\farcel\standard_library\assets\math\sum\Sum.fmu"
$metadata = Join-Path $repositoryRoot "src\farcel\standard_library\assets\categories\math\sum\block.json"
$stage = Join-Path ([System.IO.Path]::GetTempPath()) "farcel-standard-sum-build"
$gcc = Find-Gcc
...
& $gcc -shared -std=c11 -O2 -s -static-libgcc -o (Join-Path $stage "binaries\win64\FarcelSum.dll") (Join-Path $PSScriptRoot "source\sum.c")
if ($LASTEXITCODE -ne 0) { throw "GCC 未能编译 Sum.dll" }
