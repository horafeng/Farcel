...
$assetDirectory = Join-Path $repositoryRoot "src\farcel\standard_library\assets\control\pid"
$asset = Join-Path $assetDirectory "PID.fmu"
$metadata = Join-Path $assetDirectory "block.json"
$stage = Join-Path ([System.IO.Path]::GetTempPath()) "farcel-standard-pid-build"
$gcc = Find-Gcc
...
& $gcc -shared -std=c11 -O2 -s -static-libgcc -o (Join-Path $stage "binaries\win64\FarcelPid.dll") (Join-Path $PSScriptRoot "source\pid.c")
if ($LASTEXITCODE -ne 0) { throw "GCC 未能编译 PID.dll" }
