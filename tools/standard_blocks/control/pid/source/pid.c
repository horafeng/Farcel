/* Farcel Standard Block Library -- PID FMI 2.0 Co-Simulation FMU. SPDX-License-Identifier: MIT */
#include <stddef.h>
#include <stdlib.h>
...
enum { VR_KP = 0, VR_KI = 1, VR_KD = 2, VR_INITIAL_INTEGRAL = 3, VR_INITIAL_ERROR = 4, VR_U = 5, VR_Y = 6 };

typedef struct {
    fmi2Real kp;
    fmi2Real ki;
    fmi2Real kd;
    fmi2Real initial_integral;
    fmi2Real initial_error;
    fmi2Real u;
    fmi2Real integral;
    fmi2Real previous_error;
    fmi2Real y;
} PidInstance;
...
FMI2_EXPORT fmi2Status fmi2ExitInitializationMode(fmi2Component c) {
    PidInstance* x = (PidInstance*)c;
    if (!x) return fmi2Error;
    x->integral = x->initial_integral;
    x->previous_error = x->initial_error;
    x->y = x->kp * x->previous_error + x->ki * x->integral;
    return fmi2OK;
}
...
FMI2_EXPORT fmi2Status fmi2Reset(fmi2Component c) {
    PidInstance* x = (PidInstance*)c;
    if (!x) return fmi2Error;
    x->kp = 1.0; x->ki = 0.0; x->kd = 0.0; x->initial_integral = 0.0; x->initial_error = 0.0;
    x->u = 0.0; x->integral = 0.0; x->previous_error = 0.0; x->y = 0.0;
    return fmi2OK;
}
FMI2_EXPORT fmi2Status fmi2SetReal(fmi2Component c, const fmi2ValueReference refs[], size_t n, const fmi2Real values[]) {
    PidInstance* x = (PidInstance*)c; size_t i;
    if (!x) return fmi2Error;
    for (i = 0; i < n; ++i) {
        if (refs[i] == VR_KP) x->kp = values[i];
        else if (refs[i] == VR_KI) x->ki = values[i];
        else if (refs[i] == VR_KD) x->kd = values[i];
        else if (refs[i] == VR_INITIAL_INTEGRAL) x->initial_integral = values[i];
        else if (refs[i] == VR_INITIAL_ERROR) x->initial_error = values[i];
...
FMI2_EXPORT fmi2Status fmi2GetReal(fmi2Component c, const fmi2ValueReference refs[], size_t n, fmi2Real values[]) {
    ...
    if (!x) return fmi2Error;
    for (i = 0; i < n; ++i) {
        if (refs[i] == VR_KP) values[i] = x->kp;
        else if (refs[i] == VR_KI) values[i] = x->ki;
        else if (refs[i] == VR_KD) values[i] = x->kd;
        else if (refs[i] == VR_INITIAL_INTEGRAL) values[i] = x->initial_integral;
        else if (refs[i] == VR_INITIAL_ERROR) values[i] = x->initial_error;
...
FMI2_EXPORT fmi2Status fmi2DoStep(fmi2Component c, fmi2Real point, fmi2Real step, fmi2Boolean no_prior_state) {
    ...
    if (!x || step <= 0.0) return fmi2Error;
    error = x->u;
    x->integral += error * step;
    derivative = (error - x->previous_error) / step;
    x->y = x->kp * error + x->ki * x->integral + x->kd * derivative;
    x->previous_error = error;
    return fmi2OK;
}
