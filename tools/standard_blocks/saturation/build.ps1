...
$asset = Join-Path $repositoryRoot "src\farcel\standard_library\assets\math\saturation\Saturation.fmu"
$metadata = Join-Path $repositoryRoot "src\farcel\standard_library\assets\categories\math\saturation\block.json"
$stage = Join-Path ([System.IO.Path]::GetTempPath()) "farcel-standard-saturation-build"
$gcc = Find-Gcc
...
& $gcc -shared -std=c11 -O2 -s -static-libgcc -o (Join-Path $stage "binaries\win64\FarcelSaturation.dll") (Join-Path $PSScriptRoot "source\saturation.c")
if ($LASTEXITCODE -ne 0) { throw "GCC 未能编译 Saturation.dll" }
