/* Farcel Standard Block Library -- Saturation FMI 2.0 Co-Simulation FMU. SPDX-License-Identifier: MIT */
#include <stddef.h>
#include <stdlib.h>
...
enum { VR_LOWER_LIMIT = 0, VR_UPPER_LIMIT = 1, VR_U = 2, VR_Y = 3 };

typedef struct { fmi2Real lower_limit; fmi2Real upper_limit; fmi2Real u; } SaturationInstance;

static fmi2Real saturation_output(const SaturationInstance* instance) {
    if (instance->u < instance->lower_limit) return instance->lower_limit;
    if (instance->u > instance->upper_limit) return instance->upper_limit;
    return instance->u;
}
...
FMI2_EXPORT fmi2Component fmi2Instantiate(fmi2String name, fmi2Type type, fmi2String guid, fmi2String resource, const fmi2CallbackFunctions* callbacks, fmi2Boolean visible, fmi2Boolean logging) {
    SaturationInstance* instance;
    (void)name; (void)guid; (void)resource; (void)callbacks; (void)visible; (void)logging;
    if (type != fmi2CoSimulation) return NULL;
    instance = (SaturationInstance*)calloc(1, sizeof(SaturationInstance));
    if (instance != NULL) { instance->lower_limit = -1.0; instance->upper_limit = 1.0; }
    return instance;
}
...
FMI2_EXPORT fmi2Status fmi2Reset(fmi2Component c) { SaturationInstance* x = (SaturationInstance*)c; if (!x) return fmi2Error; x->lower_limit = -1.0; x->upper_limit = 1.0; x->u = 0.0; return fmi2OK; }
FMI2_EXPORT fmi2Status fmi2SetReal(fmi2Component c, const fmi2ValueReference refs[], size_t n, const fmi2Real values[]) {
    SaturationInstance* x = (SaturationInstance*)c; size_t i;
    if (!x) return fmi2Error;
    for (i = 0; i < n; ++i) { if (refs[i] == VR_LOWER_LIMIT) x->lower_limit = values[i]; else if (refs[i] == VR_UPPER_LIMIT) x->upper_limit = values[i]; else if (refs[i] == VR_U) x->u = values[i]; else return fmi2Error; }
    return fmi2OK;
}
FMI2_EXPORT fmi2Status fmi2GetReal(fmi2Component c, const fmi2ValueReference refs[], size_t n, fmi2Real values[]) {
    SaturationInstance* x = (SaturationInstance*)c; size_t i;
    if (!x) return fmi2Error;
    for (i = 0; i < n; ++i) { if (refs[i] == VR_LOWER_LIMIT) values[i] = x->lower_limit; else if (refs[i] == VR_UPPER_LIMIT) values[i] = x->upper_limit; else if (refs[i] == VR_U) values[i] = x->u; else if (refs[i] == VR_Y) values[i] = saturation_output(x); else return fmi2Error; }
    return fmi2OK;
}
