/* Farcel Standard Block Library -- Abs FMI 2.0 Co-Simulation FMU. SPDX-License-Identifier: MIT */
#include <stddef.h>
#include <stdlib.h>
...
typedef struct { fmi2Real u; } AbsInstance;
...
FMI2_EXPORT fmi2Component fmi2Instantiate(fmi2String name, fmi2Type type, fmi2String guid, fmi2String resource, const fmi2CallbackFunctions* callbacks, fmi2Boolean visible, fmi2Boolean logging) { (void)name; (void)guid; (void)resource; (void)callbacks; (void)visible; (void)logging; return type == fmi2CoSimulation ? calloc(1, sizeof(AbsInstance)) : NULL; }
...
FMI2_EXPORT fmi2Status fmi2Reset(fmi2Component c) { if (!c) return fmi2Error; ((AbsInstance*)c)->u = 0.0; return fmi2OK; }
FMI2_EXPORT fmi2Status fmi2SetReal(fmi2Component c, const fmi2ValueReference refs[], size_t n, const fmi2Real values[]) { AbsInstance* x = (AbsInstance*)c; size_t i; if (!x) return fmi2Error; for (i = 0; i < n; ++i) { if (refs[i] == VR_U) x->u = values[i]; else return fmi2Error; } return fmi2OK; }
FMI2_EXPORT fmi2Status fmi2GetReal(fmi2Component c, const fmi2ValueReference refs[], size_t n, fmi2Real values[]) { AbsInstance* x = (AbsInstance*)c; size_t i; if (!x) return fmi2Error; for (i = 0; i < n; ++i) { if (refs[i] == VR_U) values[i] = x->u; else if (refs[i] == VR_Y) values[i] = x->u < 0.0 ? -x->u : x->u; else return fmi2Error; } return fmi2OK; }
