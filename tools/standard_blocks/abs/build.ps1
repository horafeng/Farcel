...
$asset = Join-Path $repositoryRoot "src\farcel\standard_library\assets\math\abs\Abs.fmu"
$metadata = Join-Path $repositoryRoot "src\farcel\standard_library\assets\categories\math\abs\block.json"
$stage = Join-Path ([System.IO.Path]::GetTempPath()) "farcel-standard-abs-build"
$gcc = Find-Gcc
...
& $gcc -shared -std=c11 -O2 -s -static-libgcc -o (Join-Path $stage "binaries\win64\FarcelAbs.dll") (Join-Path $PSScriptRoot "source\abs.c")
if ($LASTEXITCODE -ne 0) { throw "GCC 未能编译 Abs.dll" }
