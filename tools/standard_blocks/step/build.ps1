...
$asset = Join-Path $repositoryRoot "src\farcel\standard_library\assets\sources\step\Step.fmu"
$metadata = Join-Path $repositoryRoot "src\farcel\standard_library\assets\categories\sources\step\block.json"
$stage = Join-Path ([System.IO.Path]::GetTempPath()) "farcel-standard-step-build"
$gcc = Find-Gcc
...
& $gcc -shared -std=c11 -O2 -s -static-libgcc -o (Join-Path $stage "binaries\win64\FarcelStep.dll") (Join-Path $PSScriptRoot "source\step.c")
if ($LASTEXITCODE -ne 0) { throw "GCC 未能编译 Step.dll" }
