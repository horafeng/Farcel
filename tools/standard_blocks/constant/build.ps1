...
function Find-Gcc {
    if ($env:FARCEL_GCC -and (Test-Path -LiteralPath $env:FARCEL_GCC)) {
        return $env:FARCEL_GCC
    }
    ...
    if (Test-Path -LiteralPath $wingetGcc) {
        return $wingetGcc
    }
...
$asset = Join-Path $repositoryRoot "src\farcel\standard_library\assets\sources\constant\Constant.fmu"
$metadata = Join-Path $repositoryRoot "src\farcel\standard_library\assets\categories\sources\constant\block.json"
$stage = Join-Path ([System.IO.Path]::GetTempPath()) "farcel-standard-constant-build"
$gcc = Find-Gcc
...
& $gcc -shared -std=c11 -O2 -s -static-libgcc -o (Join-Path $stage "binaries\win64\FarcelConstant.dll") (Join-Path $PSScriptRoot "source\constant.c")
if ($LASTEXITCODE -ne 0) { throw "GCC 未能编译 Constant.dll" }
